        ...
        respuesta.enqueue(new Callback<ArrayList<UsersResponse>>() {
            ...
            public void onResponse(Call<ArrayList<UsersResponse>> call, Response<ArrayList<UsersResponse>> response) {
                ...
                for (UsersResponse user : usuarios) {
                    ...
                    usuario.setDireccion(user.getAddress().getCalle());
                    usuario.setLatitud(Double.parseDouble(user.getAddress().getGeo().getLatitud()));
                    usuario.setLongitud(Double.parseDouble(user.getAddress().getGeo().getLongitud()));
                    lista.add(usuario);
                }
