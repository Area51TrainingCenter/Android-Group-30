...
public class SentenciaSQL {
    ...
    public static int nextMascota() {
        Realm realm = Realm.getDefaultInstance();

        long cantidad = realm.where(MascotaEntidad.class)
                .count();
        return ((int) cantidad + 1);
    }
    ...
    public static int obtenerUsuarioPorNombre(String nombre) {
        Realm realm = Realm.getDefaultInstance();
        UsuarioEntidad usuarioEntidad = realm.where(UsuarioEntidad.class)
                .equalTo("nombre", nombre)
                .findFirst();
        return usuarioEntidad != null ? usuarioEntidad.getId() : 0;
    }
}
